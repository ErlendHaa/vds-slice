...
static const std::unique_ptr<CoordinateToVDSVoxelTransformer>
get_coordinate_system(const Axis axis,
...
int VDSHandle::convert_ijk_to_voxel_axis_id(const int ijk_axis_id) const {
    const OpenVDS::IntVector3& mapping =
        this->ijk_coordinate_transformer_.IJKToVoxelDimensionMap();
...
requestdata VDSHandle::requestdata_from_requested_data(
...
requestdata VDSHandle::finalize_request(
...
void VDSHandle::check_axis(const int dimension,
                           char const* const expected_name) const {
    char const* const actual_name = this->layout_->GetDimensionName(dimension);
    if (std::strcmp(actual_name, expected_name)) {
        const std::string msg =
            std::string("Unsupported axis ordering in VDS for axis nr. ") +
            std::to_string(dimension) + " named " + actual_name +
...
void VDSHandle::validate_axes_order() const {
    ...
    check_axis(internal::VDSAxisID::Crossline,
               OpenVDS::KnownAxisNames::Crossline());

    const auto z =
        this->layout_->GetDimensionName(internal::VDSAxisID::DepthSampleTime);

    auto isoneof = [z](const char* x) { return !std::strcmp(x, z); };

    using namespace internal::constants::annotation;
    if (not std::any_of(axis_labels.begin(), axis_labels.end(), isoneof)) {
        const char* actual_name = this->layout_->GetDimensionName(
            internal::VDSAxisID::DepthSampleTime);
        const std::string msg =
            std::string(
                "Unsupported axis ordering in VDS for axis nr. 0 named ") +
            actual_name + ", expected " + "Depth, Time, or Sample";
        throw std::runtime_error(msg);
    }
}
...
IntPointList VDSHandle::get_bounding_box_in_index_coordinates() const {
...
DoublePointList VDSHandle::convert_ijk_to_world(
    const IntPointList& points_as_ijk) const {
    DoublePointList tmp;
    std::for_each(points_as_ijk.begin(), points_as_ijk.end(),
                  [this, &tmp](const std::pair<int, int>& point) {
                      const auto p =
                          this->ijk_coordinate_transformer_.IJKIndexToWorld(
...
IntPointList VDSHandle::convert_ijk_to_annotation(
    const IntPointList& points_as_ijk) const {
...
int VDSHandle::get_voxel_axis_id_of(const Axis axis) const {
    ...
    const auto coordinate_system =
        internal::get_coordinate_system(axis, this->layout_);
    return this->convert_ijk_to_voxel_axis_id(
        coordinate_system->axis_to_vds_dimension());
}

requestdata VDSHandle::request_volume_trace(
    const std::unique_ptr<float[][OpenVDS::Dimensionality_Max]>& coordinates,
    const std::size_t npoints,
    const InterpolationMethod interpolation_method) {
    // TODO: Verify that trace dimension is always 0
    const std::size_t size = this->access_manager_.GetVolumeTracesBufferSize(
        npoints, VDSLevelOfDetailID::Level0);
...
VDSHandle::VDSHandle(std::string url, std::string credentials) {
    ...
    this->ijk_coordinate_transformer_ =
        OpenVDS::IJKCoordinateTransformer(this->layout_);
    seismic_channel_id_ =
        layout_->GetChannelIndex(seismic_channel_name_.c_str());
...
std::string VDSHandle::get_format_string_of_seismic_channel() const {
    ...
    const VolumeDataFormat format =
        layout_->GetChannelFormat(seismic_channel_id_);
    switch (format) {
...
nlohmann::json VDSHandle::get_axis_metadata(
    const Axis axis,
    const int requested_dimension) const {
...
nlohmann::json VDSHandle::get_shape_metadata(const int npoints) const {
    return nlohmann::json::array(
        {npoints, layout_->GetDimensionNumSamples(
                      internal::VDSAxisID::DepthSampleTime)});
}

BoundingBox VDSHandle::get_bounding_box() const {
    BoundingBox bounding_box;

    bounding_box.index = this->get_bounding_box_in_index_coordinates();
    bounding_box.annotation =
        this->convert_ijk_to_annotation(bounding_box.index);
...
requestdata VDSHandle::get_slice_of(const Axis axis, const int line_number) {
    const auto coordinate_system =
        internal::get_coordinate_system(axis, this->layout_);
    internal::VoxelBounds vb = coordinate_system->get_slice_bounds_from(
        line_number, this->get_voxel_axis_id_of(axis));
    return this->request_volume_subset(std::move(vb));
}

requestdata VDSHandle::get_fence_of(
    const enum CoordinateSystem coordinate_system,
    const float* coordinates,
    const size_t npoints,
    const enum InterpolationMethod interpolation_method) {
    const auto request_coordinates = this->get_fence_request_coordinates(
        coordinate_system, coordinates, npoints, interpolation_method);

    return this->request_volume_trace(request_coordinates, npoints,
                                      interpolation_method);
}

requestdata VDSHandle::request_volume_subset(
    const internal::VoxelBounds& voxel_bounds) {
    ...
    const int size = access_manager_.GetVolumeSubsetBufferSize(
        voxel_bounds.lower, voxel_bounds.upper, format,
        VDSLevelOfDetailID::Level0, this->seismic_channel_id_);

    std::unique_ptr<char[]> data(new char[size]());
    const auto request = access_manager_.RequestVolumeSubset(
        data.get(), size, OpenVDS::Dimensions_012, VDSLevelOfDetailID::Level0,
        this->seismic_channel_id_, voxel_bounds.lower, voxel_bounds.upper,
        format);

    return finalize_request(request, "Failed to fetch slice from VDS", data,
                            size);
}
