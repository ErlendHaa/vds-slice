 ...
 *            v
 */
func TestAttributesAllAligned(t *testing.T) {
	fill := float32(-999.25)

	targetAttributes := []string{ "min", "max", "mean", "rms"}
	expected := [][]float32{
		{ -4.0,     -3.0,     -14.0,     0.0,    fill, -24.0,      fill, fill }, // min
		{  3.0,      4.0,       0.0,    14.0,    fill,  -4.0,      fill, fill }, // max
		{ -0.5,      0.5,      -7.0,     7.0,    fill, -15.0,      fill, fill }, // mean
		{  2.345208, 2.345208,  8.3666,  8.3666, fill,  16.248077, fill, fill }, // rms
	} 
